...
public class AmazfitGTS2eFirmwareInfo extends HuamiFirmwareInfo {
    ...
    public AmazfitGTS2eFirmwareInfo(byte[] bytes) {
        super(bytes);
    }
    ...
    protected HuamiFirmwareType determineFirmwareType(byte[] bytes) {
        ...
        if (ArrayUtils.equals(bytes, FW_HEADER, FW_OFFSET)) {
            if (searchString32BitAligned(bytes, "Amazfit GTS 2e")) {
    ...
    public boolean isGenerallyCompatibleWith(GBDevice device) {
        return isHeaderValid() && device.getType() == DeviceType.AMAZFITGTS2E;
    }
