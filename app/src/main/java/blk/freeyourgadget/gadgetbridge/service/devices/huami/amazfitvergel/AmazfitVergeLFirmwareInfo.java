/*  Copyright (C) 2017-2021 Andreas Shimokawa, angelpup, Daniele Gobbetti,
    Dmytro Bielik
...
package blk.freeyourgadget.gadgetbridge.service.devices.huami.amazfitvergel;
...
public class AmazfitVergeLFirmwareInfo extends HuamiFirmwareInfo {
    ...
    public AmazfitVergeLFirmwareInfo(byte[] bytes) {
        super(bytes);
    }
    ...
    protected HuamiFirmwareType determineFirmwareType(byte[] bytes) {
        ...
        if (ArrayUtils.equals(bytes, FW_HEADER, FW_OFFSET)) {
            if (searchString32BitAligned(bytes, "Amazfit Verge L")) {
    ...
    public boolean isGenerallyCompatibleWith(GBDevice device) {
        return isHeaderValid() && device.getType() == DeviceType.AMAZFITVERGEL;
    }
